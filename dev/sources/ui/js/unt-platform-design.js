...
(function (unt) {
    unt.preloader = function () {
        return {
            show: function () {
                document.getElementsByClassName('unt-loader')[0].style.display = '';
            },
            start: function () {
                let loader = unt.loader();

                loader.width = 40;
                loader.height = 40;

                let element = loader.build();
                element.style.paddingTop = '100px';

                document.getElementsByClassName('loading-spinner')[0].appendChild(element);
            },
            end: function () {
                document.getElementsByClassName('loading-spinner')[0].innerHTML = '';
            },
            error: function () {
                let loader = document.getElementsByClassName('loading-spinner')[0];
                let button = document.getElementsByClassName('retry-button')[0];

                loader.style.display = 'none';
                button.style.display = '';
                button.addEventListener('click', function () {
                    if (button.getElementsByClassName('retry-button-item')[0].classList.contains('disabled')) return;

                    button.getElementsByClassName('retry-button-item')[0].classList.add('disabled');
                    unt.toast({html: 'Reloading...'});

                    setTimeout(function () {
                        return window.location.reload();
                    }, 1000);
                });
                console.log('[!] Failed to start unt platform.');

                unt.toast({html: 'Failed to load platform.'});
            },
            success: function (goTo) {
                this.hide();
                unt.changeWindowTo(goTo);
            },
            hide: function () {
                document.getElementsByClassName('unt-loader')[0].style.display = 'none';
            }
        };
    }
})(unt);
    ...
    unt.loader = function () {
        return {
            ...
            build: function () {
                ...
                element.innerHTML =
                    '<svg width="' + this.width + '" height="' + this.height + '" viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg" stroke="#42A5F5">\n' +
...
try {
    ...
    console.log('[SW] Failed to init ServiceWorker.');
}

window.addEventListener('popstate', function () {
    unt.setModule(window.location.href.split(window.location.host)[1], true);
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        unt.users.getLanguage().then(function () {
            unt.users.get().then(function (user) {
                try {
                    unt.users.current = user;

                    unt.preloader().end();

                    setTimeout(function () {
                        unt.preloader().success(1);
                        unt.setModule(window.location.pathname, false);
                    }, 500);
                } catch (e) {
                    console.log(e);
                    unt.preloader().error();

			...
			if (msgObject.type === 'service_message') {
				...
				if (msgStringElement.innerText.match('%who%')) {
					unt.settings.users.get(msgObject.from_id).then(function (user) {
						...
						msgStringElement.innerHTML = msgStringElement.innerHTML.replace('%who%', '<b><a target="_blank" href=' + (user.screen_name || (user.account_type === 'bot' ? ('bot' + user.bot_id) : ('id' + user.user_id))) + '"/public" style="color: black">' + (
				...
				if (msgStringElement.innerText.match('%when%')) {
					unt.settings.users.get(msgObject.event.to_id || 0).then(function (user) {
						...
						msgStringElement.innerHTML = msgStringElement.innerHTML.replace('%when%', '<b><a target="_blank" href=' + (user.screen_name || (user.account_type === 'bot' ? ('bot' + user.bot_id) : ('id' + user.user_id))) + '"/public" style="color: black">' + (

...
if (!($current_user_level < 1 || !$context->isLogged()))
{
	require __DIR__ . '/../../bin/functions/management.php';
	if (isset($_POST['action']) && $current_user_level > 0)
	{
		...
		if ($action === 'toggle_verification_state')
		{
		...
		if ($action === 'toggle_online_show_state')
		{
		...
		if ($action === "toggle_project_close")
		{
			if ($current_user_level >= 4)
			{
		...
		if ($action === "toggle_register_close")
		{
			if ($current_user_level >= 4)
			{

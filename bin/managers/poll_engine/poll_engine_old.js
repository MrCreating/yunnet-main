...
// run external server
utils.cache.connect('memcached:11211', function (e, c) {
	if (e)
		return console.log('\x1b[31m[!]\x1b[0m Failed to start LP!');
	
	utils.fs.readFile('/home/unt/config/local/config/nginx/privkey.pem', null, function (err, key) {
		utils.fs.readFile('/home/unt/config/local/config/nginx/fullchain.pem', null, function (err, cert) {
			https.createServer({
				key: key,
				cert: cert
